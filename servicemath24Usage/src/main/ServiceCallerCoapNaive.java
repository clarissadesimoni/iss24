...
public class ServiceCallerCoapNaive {
	...
	public void doJob() {
		try {
			...
			if (answer != null)
				CommUtils.outblue("ServiceCallerCoapNaive|answer=" + answer);
		} catch (Exception e) {
	...
	protected String sendUsingCoap(IApplMessage req) {
		try {
			...
			/* 10 */ response = client.put(req.toString(), MediaTypeRegistry.TEXT_PLAIN);
	...
	protected String getAnswer(CoapResponse response) {
		if (response != null) {
			CommUtils.outblue("ServerCallerCoapNaive|response=" + response);
			// ACK-2.05 ...
